  ...
  return (
    <PageShell testId="docs-page">
      ...
      <section className="relative max-w-6xl mx-auto px-6 lg:px-10 pb-16">
        {categories.map((cat) => {
          ...
          return (
            <div key={cat} className="mb-12 last:mb-0">
              <span className="font-mono text-[11px] uppercase tracking-widest text-nua-ink2">{cat} modules</span>
                ...
                {items.map((d, i) => (
                  ...
                  >
                    <div className="flex items-start justify-between">
                      <div className="w-9 h-9 rounded-lg flex items-center justify-center" style={{ background: `${d.color}0D` }}>
                        <d.icon className="w-4 h-4" style={{ color: '#750D28' }} />
                      </div>
                      <ArrowUpRight className="w-4 h-4 text-nua-ink2 opacity-0 group-hover:opacity-100 transition-opacity" />
                    </div>
                    <h3 className="mt-4 font-display font-semibold text-nua-ink text-sm">{d.title}</h3>
                    <p className="mt-1.5 text-[13px] text-nua-ink2 leading-relaxed">{d.summary}</p>
                    <span className="mt-3 inline-block font-mono text-[10px] uppercase tracking-widest text-nua-ink2">
        ...
        <div className="mt-4 flex flex-col sm:flex-row items-center justify-between gap-6 rounded-2xl border border-nua-border bg-nua-bgAlt p-6 sm:p-8">
          <div>
            <div className="font-display text-lg font-semibold text-nua-ink">Can't find what you're looking for?</div>
            <div className="text-sm text-nua-ink2 mt-1">Browse the FAQ, or email our team directly and we'll walk you through it.</div>
          </div>

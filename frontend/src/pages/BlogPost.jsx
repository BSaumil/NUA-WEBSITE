...
function ContentBlock({ block }) {
  switch (block.type) {
    case "h2":
      return (
        <h2 className="mt-12 font-display text-2xl sm:text-3xl font-bold text-nua-ink tracking-tight">
          {block.text}
        </h2>
      );
    case "h3":
      return (
        <h3 className="mt-8 font-display text-lg font-semibold text-nua-ink">
          {block.text}
        </h3>
      );
    case "ul":
    ...
    case "stat":
      return (
        <div className="my-8 rounded-2xl bg-nua-surface border border-nua-border p-6 sm:p-8 flex flex-col items-center text-center">
          <div className="font-display text-4xl sm:text-5xl font-bold text-nua-ink">{block.value}</div>
          <div className="mt-1 font-mono text-[11px] uppercase tracking-wider text-nua-ink2 max-w-[280px]">
            {block.label}
          </div>
        </div>
      );
  ...
  return (
    <PageShell testId="blog-post-page">
      ...
      <div className="relative max-w-3xl mx-auto px-6 lg:px-10 pb-24 lg:pb-32">
        <div className="flex items-center gap-4 font-mono text-[11px] text-nua-ink2">
        ...
        {post.faqs?.length > 0 && (
          <div className="mt-16">
            <h2 className="font-display text-2xl font-bold text-nua-ink tracking-tight">Frequently asked</h2>
            <div className="mt-6 space-y-4">
              {post.faqs.map((f) => (
                <div key={f.q} className="rounded-2xl bg-nua-surface border border-nua-border p-5 sm:p-6">
                  <div className="font-display font-semibold text-nua-ink text-sm">{f.q}</div>
                  <p className="mt-2 text-[14px] text-nua-ink2 leading-relaxed">{f.a}</p>
                </div>
              ))}
            </div>
          </div>
        ...
        {related.length > 0 && (
          <div className="mt-16">
            <h2 className="font-display text-2xl font-bold text-nua-ink tracking-tight">Related reading</h2>
            <div className="mt-6 grid sm:grid-cols-2 gap-4">
              {related.map((r) => (
                ...
                >
                  ...
                  <p className="mt-2 text-[13px] text-nua-ink2 leading-relaxed line-clamp-2">{r.excerpt}</p>
                </Link>
              ))}
            </div>
          </div>

  ...
  return (
    <PageShell testId="doc-detail-page">
      ...
      <div className="relative max-w-4xl mx-auto px-6 lg:px-10 pb-24 lg:pb-32">
        {/* Header strip */}
        <div className="flex flex-wrap items-center gap-4 rounded-2xl border p-5" style={{ background: `linear-gradient(135deg, ${data.color}0D, #FFFDF9)`, borderColor: '#E8DED4' }}>
          <div className="w-11 h-11 rounded-xl flex items-center justify-center border flex-shrink-0" style={{ background: `${data.color}0D`, borderColor: '#E8DED4' }}>
            <Icon className="w-5 h-5" style={{ color: '#750D28' }} />
          </div>
          <div>
            <div className="font-display font-semibold text-nua-ink">{data.title} setup guide</div>
            <div className="mt-0.5 flex items-center gap-1.5 font-mono text-[11px] text-nua-ink2">
        ...
        {/* Prerequisites */}
        <div className="mt-10">
          <span className="font-mono text-[10px] uppercase tracking-widest text-nua-ink2">Before you start</span>
          <div className="mt-4 rounded-2xl bg-nua-surface border border-nua-border p-6 sm:p-7">
        ...
        {/* Steps */}
        <div className="mt-10">
          <span className="font-mono text-[10px] uppercase tracking-widest text-nua-ink2">Setup steps</span>
          <ol className="mt-4 space-y-4">
            {data.steps.map((s, i) => (
              <motion.li
                ...
                className="flex gap-4 rounded-2xl bg-nua-surface border border-nua-border p-5 sm:p-6"
              >
                <span
                ...
                <div>
                  <h3 className="font-display font-semibold text-nua-ink text-sm">{s.title}</h3>
                  <p className="mt-1.5 text-[13px] text-nua-ink2 leading-relaxed">{s.body}</p>
                </div>
              </motion.li>
            ))}
          </ol>
        ...
        <div className="mt-10 grid lg:grid-cols-2 gap-5">
          <div className="rounded-2xl bg-nua-surface border border-nua-border p-6 sm:p-7">
            <div className="flex items-center gap-2">
              <CheckCircle2 className="w-4 h-4 text-nua-burgundy" />
              <span className="font-mono text-[10px] uppercase tracking-widest text-nua-ink2">Tips</span>
            </div>
            <ul className="mt-4 space-y-3">
              {data.tips.map((t) => (
                <li key={t} className="text-[13px] text-nua-ink leading-relaxed">{t}</li>
              ))}
            </ul>
          </div>
          <div className="rounded-2xl bg-nua-surface border border-nua-border p-6 sm:p-7">
            <div className="flex items-center gap-2">
              <AlertTriangle className="w-4 h-4 text-nua-burgundy" />
              <span className="font-mono text-[10px] uppercase tracking-widest text-nua-ink2">Troubleshooting</span>
            </div>
            <ul className="mt-4 space-y-4">
              {data.troubleshooting.map((t) => (
                <li key={t.issue}>
                  <div className="text-[13px] font-semibold text-nua-ink">{t.issue}</div>
                  <div className="mt-1 text-[13px] text-nua-ink2 leading-relaxed">{t.fix}</div>
                </li>
              ))}
            </ul>
          </div>
        ...
        {related.length > 0 && (
          <div className="mt-10">
            <span className="font-mono text-[10px] uppercase tracking-widest text-nua-ink2">Related modules</span>
            <div className="mt-4 grid sm:grid-cols-3 gap-4">
              {related.map((r) => (
                ...
                >
                  <div className="w-9 h-9 rounded-lg flex items-center justify-center" style={{ background: `${r.color}0D` }}>
                    <r.icon className="w-4 h-4" style={{ color: '#750D28' }} />
                  </div>
                  <h3 className="mt-3 font-display font-semibold text-nua-ink text-sm">{r.title}</h3>
                  <p className="mt-1 text-[13px] text-nua-ink2 leading-relaxed">{r.tagline}</p>
                </Link>
              ))}
            </div>
          </div>
        ...
        {/* CTA */}
        <div className="mt-10 flex flex-col sm:flex-row items-center justify-between gap-6 rounded-2xl border border-nua-border bg-nua-bgAlt p-6 sm:p-8">
          <div>
            <div className="font-display text-lg font-semibold text-nua-ink">Still stuck on {data.title.toLowerCase()} setup?</div>
            <div className="text-sm text-nua-ink2 mt-1">Email us directly, a real person on the team replies.</div>
          </div>

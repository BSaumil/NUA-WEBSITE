...
const outcomes = [
  { icon: CheckCircle2, label: "Executed", range: "95%+ confidence", body: "Acts automatically, no wait.", color: "#157E3C" },
  { icon: Sparkles, label: "Approved-pending", range: "75–94% confidence", body: "One-tap approval needed.", color: "#7D52DD" },
  { icon: Clock, label: "Suggested", range: "Below 75%", body: "Surfaced, never auto-actioned.", color: "#A45D0D" },
];
...
const auditStatusColor = { executed: "#157E3C", approved: "#7D52DD", suggested: "#A45D0D" };
    ...
    <PageShell testId="ai-agent-page">
      ...
      <section className="relative max-w-6xl mx-auto px-6 lg:px-10 pb-20">
        <div className="grid lg:grid-cols-2 gap-10 items-center">
          <div>
            <h2 className="font-display text-2xl sm:text-3xl font-bold text-nua-ink tracking-tight">The decision loop.</h2>
            <p className="mt-3 text-nua-ink2 leading-relaxed">
            ...
            <div className="mt-6 space-y-4">
              {principles.map((p) => (
                <div key={p.title} className="flex items-start gap-3">
                  ...
                  <div>
                    <div className="font-display font-semibold text-nua-ink text-sm">{p.title}</div>
                    <div className="text-sm text-nua-ink2 mt-0.5 leading-relaxed">{p.body}</div>
                  </div>
                </div>
              ))}
            </div>
            ...
            <div className="relative pl-11">
              ...
              {steps.map((s) => (
                <div key={s.title} className="relative flex items-start gap-3 pb-6 last:pb-0">
                  ...
                  <div>
                    <div className="font-display font-semibold text-nua-ink text-sm">{s.title}</div>
                    <div className="text-[13px] text-nua-ink2 mt-0.5 leading-relaxed">{s.body}</div>
                  </div>
                </div>
              ))}
            </div>

            <div className="mt-2 pt-5 border-t border-nua-border">
              <span className="font-mono text-[10px] uppercase tracking-widest text-nua-ink2">Then, one of three outcomes</span>
      ...
      {/* Forecasting */}
      <section id="forecasting" data-testid="ai-agent-forecasting-section" className="relative py-20 lg:py-28 bg-nua-bgAlt text-nua-ink">
        <div className="relative max-w-6xl mx-auto px-6 lg:px-10">
          <div className="grid lg:grid-cols-2 gap-10 items-center">
            <div>
              <span className="font-mono text-[11px] uppercase tracking-widest text-nua-burgundy">Forecasting</span>
            ...
            <motion.div
              ...
              viewport={{ once: true }}
              transition={{ duration: 0.5 }}
              className="rounded-2xl bg-white border border-nua-border shadow-[0_30px_60px_-20px_rgba(0,0,0,0.08)] p-6"
            >
              <div className="flex items-center justify-between mb-4">
                <div>
                  <div className="font-display font-semibold text-sm">Covers forecast · next 14 days</div>
                  <div className="font-mono text-[11px] text-nua-muted">Actual vs predicted, with confidence band</div>
                </div>
              ...
              <div className="flex items-center justify-between font-mono text-[10px] uppercase tracking-wider text-nua-muted">
                <span>← actual</span>
                <span className="text-nua-burgundy">predicted →</span>
              </div>
            </motion.div>
          </div>
        </div>
      ...
      <section id="automation-library" data-testid="ai-agent-automation-library-section" className="relative py-20 lg:py-28 bg-nua-bg">
        ...
        <div className="relative max-w-6xl mx-auto px-6 lg:px-10">
          <div className="max-w-2xl">
            <span className="font-mono text-[11px] uppercase tracking-widest text-nua-burgundy">Automation library</span>
          ...
          <div className="mt-8 grid sm:grid-cols-2 lg:grid-cols-3 gap-4">
            {automations.map((a, i) => (
              ...
              >
                ...
                <h3 className="mt-4 font-display font-semibold text-nua-ink text-sm">{a.name}</h3>
                <p className="mt-1.5 text-[13px] text-nua-ink2 leading-relaxed">{a.body}</p>
              </motion.div>
            ))}
          </div>
        </div>
      ...
      {/* Audit trail */}
      <section id="audit-trail" data-testid="ai-agent-audit-trail-section" className="relative py-20 lg:py-28 bg-nua-bgAlt text-nua-ink">
        <div className="relative max-w-6xl mx-auto px-6 lg:px-10">
          <div className="grid lg:grid-cols-12 gap-10">
            <div className="lg:col-span-5">
              <span className="font-mono text-[11px] uppercase tracking-widest text-nua-burgundy">Audit trail</span>
              <h2 className="font-display mt-3 text-3xl sm:text-4xl font-bold tracking-tight leading-[1.05]">
            ...
            >
              ...
              <ul className="divide-y divide-black/5">
                {auditLog.map((l, i) => (
                  ...
                  >
                    <span className="font-mono text-[10px] text-nua-muted w-16 flex-shrink-0">{l.time}</span>
                    <span className="font-mono text-[10px] uppercase tracking-wider text-nua-burgundy w-20 flex-shrink-0">{l.module}</span>
                    <span className="text-sm text-nua-ink flex-1 min-w-0 truncate">{l.action}</span>
                    <span
